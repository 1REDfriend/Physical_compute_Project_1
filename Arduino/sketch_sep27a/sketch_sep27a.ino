...
// NEW: Array of possible KWP/ISO headers to try
// ===================================================================================
const uint8_t KWP_HEADERS[][3] = {
  { 0x80, 0x11, 0xF1 },  // KWP2000 Physical (Honda Standard)
  { 0xC1, 0x33, 0xF1 },  // KWP2000 Physical with length
  { 0x68, 0x6A, 0xF1 },  // ISO 9141-2
  { 0x81, 0x11, 0xF1 }   // KWP2000 Physical variation
};
const int NUM_HEADERS = sizeof(KWP_HEADERS) / sizeof(KWP_HEADERS[0]);

uint8_t active_header[3];        // The header currently being used for requests
int current_header_index = 0;    // Index for the KWP_HEADERS array
bool ecu_has_responded = false;  // Flag to lock the header once ECU responds
// ===================================================================================
...
// MODIFIED: k_send_req now uses the global 'active_header'
...
void kline_fast_init_pulse() {
  ...
  digitalWrite(K_TX_PIN, HIGH);  // Idle State (High)
  delay(100);                    // ให้เวลาสำหรับ Idle State

  // KWP2000 Fast Init: Low Pulse 50 ms
  digitalWrite(K_TX_PIN, LOW);   // เริ่ม Low
  delay(50);                     // Low 50 ms (Wake-up)
  digitalWrite(K_TX_PIN, HIGH);  // ปล่อย High เพื่อเตรียมส่ง UART

  // Re-attach UART ที่ 10400 8N1 เพื่อส่ง Start Communication Message
...
void print_pid_decoded(uint8_t pid, const uint8_t* data, size_t n) {
  ...
  switch (pid) {
    case 0x04:
      ...
      break;
    case 0x05:
      if (n >= 1) logf("Coolant Temp: %d C\n", (int)data[0] - 40);
      break;
    case 0x0C:
      if (n >= 2) logf("RPM: %.0f rpm\n", u16(0) / 4.0);
      break;
    case 0x0D:
      if (n >= 1) logf("Speed: %d km/h\n", data[0]);
      break;
    case 0x0F:
      if (n >= 1) logf("Intake Air Temp: %d C\n", (int)data[0] - 40);
      break;
    case 0x10:
      if (n >= 2) logf("MAF: %.2f g/s\n", u16(0) / 100.0);
      break;
    case 0x11:
      if (n >= 1) logf("Throttle: %.1f %%\n", data[0] * 100.0 / 255.0);
      break;
    case 0x2F:
      if (n >= 1) logf("Fuel Level: %.1f %%\n", data[0] * 100.0 / 255.0);
      break;
    case 0x33:
      if (n >= 1) logf("Baro Pressure: %d kPa\n", data[0]);
      break;
    case 0x46:
      if (n >= 1) logf("Ambient Temp: %d C\n", (int)data[0] - 40);
      break;
    case 0x5E:
      if (n >= 2) logf("Engine Fuel Rate: %.2f L/h\n", u16(0) / 20.0);
      break;
    default:
      {
...
// request_mode01_and_print and get_supported_block now use mark_rx_ok/fail
bool request_mode01_and_print(uint8_t pid) {
  ...
  mark_rx_ok();  // We got a parsable reply
  int data_start = idx + 2;
  int data_end = (int)n - 1;
...
bool get_supported_block(uint8_t base_pid, uint8_t out_bits[4]) {
  ...
  uint8_t buf[64];
  size_t n = k_read_frame(buf, sizeof(buf));
  logf("K_Line Return block Size : (n=%u)\n" , (unsigned)n);
  ...
  if (!verify_checksum(buf, n)) { /* not fatal */
  }
  ...
  mark_rx_ok();  // We got a valid block response
...
void session_maintain() {
  ...
  // ส่ง keep-alive ทุก ~1500 ms
  if (now - last_keepalive_ms > 1500) {
    send_tester_present();
    last_keepalive_ms = now;
  }

  // ถ้าไม่มีตอบกลับติดกันหลายครั้ง → re-init
  if (no_reply_count >= 2) {
    // drain
    while (KLine.available()) (void)KLine.read();
    // re-init pulse + UART begin อีกครั้ง (ลอง 8E1 ด้วยถ้ายังเงียบ)
    kline_fast_init_pulse();
    no_reply_count = 0;
  }
}

// เรียกหลังจาก “ขอแล้วได้คำตอบ”
...
// เรียกเมื่อ “ขอแล้วไม่ตอบ/สั้นผิดปกติ”
inline void mark_rx_fail() {
  no_reply_count++;
}

void send_tester_present() {
  // service 0x3E, subfunction 0x00
  k_send_req(0x3E, 0x00);
  delay(REQ_GAP_MS);
  uint8_t buf[32];
  size_t n = k_read_frame(buf, sizeof(buf));
  // (ไม่ต้องซีเรียสกับการ parse มาก แค่ให้ ECUเห็นว่าเรายังอยู่)
}
...
void setup() {
  ...
  // NEW: Initialize the first header
...
// MODIFIED: loop() now has a discovery phase and a scanning phase
void loop() {
  net_poll_clients();

  if (!ecu_has_responded) {
    // --- DISCOVERY PHASE ---
    // Try to connect by requesting PID 0x00 with the current header
    digitalWrite(2, HIGH);
    ...
    get_supported_block(0x00, dummy_bits);  // This function will call mark_rx_ok() on success

    // If it's still not connected after the attempt, move to the next header
    if (!ecu_has_responded) {
      digitalWrite(2, LOW);
      current_header_index = (current_header_index + 1) % NUM_HEADERS;
      memcpy(active_header, KWP_HEADERS[current_header_index], 3);
      logf("No response. Moving to next header.\n");
      delay(500);  // Wait a bit before trying the next one
    }

  } else {
    // --- SCANNING PHASE ---
    // ECU has responded, so now we use the locked header to scan for PIDs.
    session_maintain();  // Maintain the session only after connection is established
